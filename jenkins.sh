...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  exec java -Duser.home="$JENKINS_HOME" -Dhttps.protocols=TLSv1.2 -Djdk.tls.client.protocols=TLSv1.2 -Dcom.ibm.jsse2.overrideDefaultTLS=true "${java_opts_array[@]}" -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}" "$@"
fi
